...
public abstract class BaseTabRenderer
{
    ...
    protected BaseTabRenderer(TitleBarTabs? parentWindow)
    {
        ...
        InitializeVirtuals(parentWindow);
    ...
    private void InitializeVirtuals(TitleBarTabs? parentWindow)
    {
        ShowAddButton = true;
    ...
    public bool IsWindows10
    {
        get
        {
            ...
            return _isWindows10 != null && _isWindows10.Value;
        }
    }
    ...
    /// <summary>
    /// Gets the RendersEntireTitleBar.
    /// </summary>
    public virtual bool RendersEntireTitleBar => false;

    /// <summary>
    /// Gets the NonClientHitTest
    /// </summary>
    ...
    protected internal virtual void Overlay_MouseDown(object? sender, MouseEventArgs e)
    {
        ...
        if (_parentWindow is not { SelectedTab: not null })
        {
            return;
        }
    ...
    public virtual TitleBarTab? OverTab(IEnumerable<TitleBarTab?> tabs, Point cursor)
    {
        ...
        foreach (TitleBarTab? tab in tabs.Where(tab => tab?.TabImage != null))
        {
            ...
            if (tab != null && tab.Active && IsOverTab(tab, cursor))
            {
                overTab = tab;
                break;
            }
    ...
    {
        return tab != null && IsOverNonTransparentArea(tab.Area, tab.TabImage, cursor);
    }
    ...
    {
        if (tab != null && (!tab.ShowCloseButton || _wasTabRepositioning))
        {
            return false;
        }
    ...
    public virtual void Render(List<TitleBarTab?>? tabs, Graphics graphicsContext, Point offset, Point cursor, bool forceRedraw = false)
    {
        ...
        if (_activeCenterImage != null)
        {
            ...
            int i = tabs.Count - 1;
            List<Tuple<TitleBarTab?, int, Rectangle>> activeTabs = new();
            ...
            if (selectedIndex != -1)
            {
                ...
                if (tabLeftImage != null)
                {
                    if (tabRightImage != null)
                    {
                                        ...
                                        dropIndex =
                                            Math.Min(
                                                Convert.ToInt32(
                                                    Math.Round(
                                                        Convert.ToDouble(
                                                            tabArea.X - SystemInformation.BorderSize.Width - offset.X - TabRepositionDragDistance) /
            ...
            {
                if (_activeLeftSideImage != null)
                {
                    if (_activeRightSideImage != null)
                    {
                        if (tabCenterImage != null)
                        {
                            if (_addButtonImage != null)
                ...
                var addButtonHoverImage = cursorOverAddButton
                    ? _addButtonHoverImage
                    : _addButtonImage;
                if (addButtonHoverImage != null)
                {
                    if (_addButtonHoverImage != null)
                    {
                        if (_addButtonImage != null)
                        {
                            var srcWidth = cursorOverAddButton
                                ? _addButtonHoverImage.Width
                                : _addButtonImage.Width;
                            graphicsContext.DrawImage(
                                addButtonHoverImage, _addButtonArea, 0, 0, srcWidth,
                                cursorOverAddButton
    ...
    protected virtual void Render(Graphics graphicsContext, TitleBarTab? tab, int index, Rectangle area, Point cursor, Image? tabLeftImage, Image? tabCenterImage, Image? tabRightImage)
    {
            ...
            if (tab != null)
            {
                if (tabCenterImage != null)
                {
                    tab.TabImage = new Bitmap(
                        area.Width <= 0
                            ? 1
                            : area.Width, tabCenterImage.Height <= 0
                            ? 1
                            : tabCenterImage.Height);
                    ...
                    if (tabLeftImage != null)
                    {
                        tabGraphicsContext.DrawImage(
                            tabLeftImage, new Rectangle(0, 0, tabLeftImage.Width, tabLeftImage.Height), 0, 0, tabLeftImage.Width, tabLeftImage.Height,
                            GraphicsUnit.Pixel);
                        ...
                        {
                            tabGraphicsContext.DrawImage(
                                tabRightImage, new Rectangle(tabLeftImage.Width + _tabContentWidth, 0, tabRightImage.Width, tabRightImage.Height), 0, 0,
                                tabRightImage.Width,
                        ...
                        if (tabRightImage != null)
                        {
                            if (closeButtonImage != null)
                            {
                                tab.CloseButtonArea = new Rectangle(
                                    area.Width - tabRightImage.Width - CloseButtonMarginRight - closeButtonImage.Width, CloseButtonMarginTop,
        ...
        if (tab is { TabImage: not null })
        {
            ...
            if (tab.Content != null && tab.Content.ShowIcon && _tabContentWidth > 16 + IconMarginLeft + (tab.ShowCloseButton
            ...
            if (tab.Content != null && _tabContentWidth > (tab.Content.ShowIcon
    ...
    internal virtual void CombineTab(TitleBarTab? tab, Point cursorLocation)
    {
        ...
        if (_parentWindow != null)
        {
            var parentWindowTabs = _parentWindow.Tabs;
            int dropIndex = parentWindowTabs.FindIndex(t => t != null && t.Area.Left <= cursorLocation.X && t.Area.Right >= cursorLocation.X);
            ...
            if (parentWindowTabs.Count > 0)
            {
                var firstOrDefault = parentWindowTabs.FirstOrDefault();
                if (firstOrDefault != null)
                {
                    _tabClickOffset = firstOrDefault.Area.Width / 2;
                }
            }
            else
            {
            ...
            if (tab != null)
            {
                ...
                if (dropIndex == -1)
                {
                    parentWindowTabs.Add(tab);
                    dropIndex = parentWindowTabs.Count - 1;
                }

                else
                {
                    parentWindowTabs.Insert(dropIndex, tab);
                }
            }
